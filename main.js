var express = require('express'),
    ...
    const pool = mysql.createPool({
      connectionLimit: 10,
      host: 'ludo-game-db.cvmkqsewy6ej.us-east-1.rds.amazonaws.com',
      port: 3306,
      user: 'admin',
      password: 'Securedb12',
      database: 'LudoGame' 
...
router.get('/createNickname', function (req, res) {
    res.render('user', {'baseUrl': config.baseUrl});
});

router.get('/admin', function (req, res) {
    // Fetch data from MySQL table 'winners'
    pool.query('SELECT * FROM winners', (error, results, fields) => {
        if (error) {
        ...
        res.render('adminpage', { winners: results, baseUrl: config.baseUrl });
    });
});

router.get('/adminlogin', function (req, res) {
    res.render('adminlogin', {'baseUrl': config.baseUrl});
});
...
router.get('/lobby', function (req, res, next) {
    res.render('playerlobby', {'baseUrl': config.baseUrl});
});

router.get('/game', function (req, res) {
    res.render('gameplay', {'baseUrl': config.baseUrl});
});
...
router.post('/rest/adminlogin', function (req, res) {
    ...
    if (req.body.username === 'admin' && req.body.password === 'secure') {
        let token = playerAuth.getAdminToken();
        res.json({
            success: true,
            playerId: 1,
            token: token
